...
class ProjectDao extends BaseDao {
	...
	private function _deleteRelativeProjectActivitiesForProject($projectId) {
       ...
       private function _deleteRelativeProjectDiusForProject($projectId) {

		try {
			$q = Doctrine_Query :: create()
				->from('ProjectDiu')
				->where('is_deleted = ?', ProjectDiu::ACTIVE_PROJECT_DIU)
				->andWhere('project_id = ?', $projectId);
			$projectActivities = $q->execute();

			foreach ($projectDius as $projectDiu) {
				$projectDiu->setIsDeleted(ProjectDIU::DELETED_PROJECT_DIU);
				$projectDiu->save();
			}
		} catch (Exception $e) {
			throw new DaoException($e->getMessage());
...
// kartik new created for diu        
        public function getProjectDiuById($DiuId) {

		try {
			return Doctrine :: getTable('ProjectDiu')->find($DiuId);
		} catch (Exception $e) {
			throw new DaoException($e->getMessage());
		}
	}
...
//                    $q = "SELECT * FROM ohrm_project_activity WHERE project_id = '".$projectId."'";
//                    $pdo = Doctrine_Manager::connection()->getDbh();
//                    $query = $pdo->prepare($q);
//                    $query->execute();
//                    $results = $query->fetchAll(PDO::FETCH_ASSOC);
...
// kartik new created for diu
        public function getDiuListByProjectId($projectId) {
		try {
			$q = Doctrine_Query :: create()
				->from ('ProjectDiu')
                                ->where('is_deleted = ?', ProjectDiu::ACTIVE_PROJECT_DIU)
...
// kartik new created for diu      
    ...
    public function searchProjects($searchClues, $allowedProjectList) {
        try {
            ...
            $q = "SELECT p.project_id As projectId, p.description as description, p.name AS projectName, c.customer_id AS customerId, c.name AS customerName, 
...
// kartik new created for diu                                                
        public function deleteProjectDius($diuId) {

		try {
			$projectDiu = Doctrine :: getTable('ProjectDiu')->find($diuId);
			$projectDiu->setIsDeleted(ProjectDiu::DELETED_PROJECT_DIU);
			$projectDiu->save();
		} catch (Exception $e) {
			throw new DaoException($e->getMessage());
        ...
        public function getDiuList($sortField='name',$sortOrder='ASC',$limit = null, $offset = null) {

//            echo "aaaaa<pre>";print_r($_SERVER);echo "</pre>";
        $projectId=  explode("/",$_SERVER['REQUEST_URI']);    
        
        if($projectId[6]=="saveProject")
        {            
...
//        echo "<pre>";print_r($projectId);echo "</pre>";
        ...
        try {
            $q = Doctrine_Query :: create()
			    ...
			    ->andwhere('project_id = ?', $projectId);
//            if ($activeOnly == true) {
////                $q->addWhere('isDeleted = ?', ProjectDiu::ACTIVE_PROJECT_DIU);
...
// kartik new created for diu
    ...
    private function _getSortField($sortFieldName){
        ...
        else if ($sortFieldName === 'projectAdminName') {
