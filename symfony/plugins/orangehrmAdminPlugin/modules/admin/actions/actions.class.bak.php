...
//kartik for add field in activity    
      public function executeListProjectDiu(sfWebRequest $request) {
        $projectService = new ProjectService();
        $this->listProject = $projectService->getProjectList();
        $currentProjectId = isset($_POST['id']) ? $_POST['id'] : $request->getParameter('id');
        $this->currentProject = $currentProjectId;

        $projectActivityList = $projectService->getProjectDiuById($currentProjectId);
        if (count($projectDiuList) > 0) {
            $this->projectDiuList = $projectDiuList;
            $this->hasProjectDiu = true;
        }else
            $this->hasProjectDiu = false;
    }
//kartik for add field in activity    
...
// kartik new created for diu    
     public function executeDeleteProjectDiu(sfWebRequest $request) {
        $projectId = $request->getParameter('id');
        if (count($request->getParameter('chkLocID')) > 0) {

            $projectService = new ProjectService();
            $projectService->deleteProjectDius($request->getParameter('chkLocID'));
            $this->setMessage('SUCCESS', array(TopLevelMessages::DELETE_SUCCESS));
        }else
            $this->setMessage('NOTICE', array(TopLevelMessages::SELECT_RECORDS));

        $this->redirect('admin/listProjectDiu?id=' . $projectId);
    }
// kartik new created for diu    
...
// kartik new created for diu    
     public function executeGetDiusRelatedToAProjectAjax(sfWebRequest $request) {

        $projectId = $request->getParameter("projectId");
        $timesheetDao = new TimesheetDao();
        $this->noProjectDiu = "no";

        if ($projectId == -1) {
            $this->diuList = "All";
        } else {
            $this->diuList = $timesheetDao->getProjectDiusByPorjectId($projectId);
            $this->allDiuList = $timesheetDao->getProjectDiusByPorjectId($projectId, true);
            if ($this->allDiuList == null) {
                $this->noProjectDiu = "yes";
            }
        }
    }
// kartik new created for diu
