...
class TimesheetService {
    ...
    public function saveTimesheetItems($inputTimesheetItems, $employeeId, $timesheetId, $keysArray, $initialRows,$ot) {
        ...
        foreach ($inputTimesheetItems as $inputTimesheetItem) {
            ...
            if ($activityId != null) {
                ...
                for ($i = 0; $i < sizeof($keysArray); $i++) {
                    ...
                    if ($timesheetItemId != null) {
                        ...
                        if ($existingTimesheetItem[0]->getProjectId() != null) {
                            ...
                            /**        
                                By:kartik gondalia
                                Date:23-03-2013
                                Purpose:Add for config variables,for OT.
                        ...
                        $select="select date,timesheet_id,project_id,activity_id from ohrm_timesheet_item  where timesheet_id ='".$timesheetId."' AND date='".$keysArray[$i]."'AND project_id='".$projectId."' AND activity_id='".$activityId."'";                                            
                        $res_select=mysql_query($select);                        
                        $count_select = mysql_num_rows($res_select);                            
                        $count_select_obj = mysql_fetch_object($res_select);                                                                                                                                                 
                    ...
                    $ot[$k][$i]=$this->convertDurationToSeconds($ot[$k][$i]);
                    $sql =  "UPDATE ohrm_timesheet_item SET duration_ot ='".$ot[$k][$i]."'  where timesheet_id ='".$timesheetId."' AND date='".$keysArray[$i]."'AND project_id='".$projectId."' AND activity_id='".$activityId."'";                     
                    $res = mysql_query($sql);                    
                }
            }$k++;
...
//                         exit;
    ...
    /**        
        ...
        Keep Original line here if:-
    */
     public function convertDurationToHours_ot($durationInSecs) {
...
/*===================================================================================================*/
    ...
    public function convertDurationToSeconds($duration) {
        ...
        if ($pos !== false) {
            $str_time = $duration;
            //rushika 20130702
            sscanf($str_time, "%d:%s:%d", $hours, $minutes, $seconds);
...
//  jugni have madde changes 2030621
        $month=date('m');
        $year=date('Y');
	    ...
	    $j+=7;
            if(date("Y-m-d",  strtotime($currentDate)) >= date("Y-m-d",strtotime($startDate)) && date("Y-m-d",strtotime($currentDate)) <= date("Y-m-d",strtotime($endDate1)))
            {
                $timesheetStartingDate=$startDate;
                $endDate=$endDate1;
                break;
            }
        }
...
//     jugni have made changes 20130621
     public function createSlot($employeeId,$selectedYear,$selectedMonth) { 
	  $val=$employeeId."/".$selectedMonth."/".$selectedYear;
	  return $val;
     }
