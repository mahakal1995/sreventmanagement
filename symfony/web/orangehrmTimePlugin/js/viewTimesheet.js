...
//     jugni have made changes 20130621
    $("#btnCreateTimesheet").click(function(){      
     selectedYear=$("#drpyear").val();
     selectedMonth=$("#drpmonth").val();
     if(selectedYear<=2013 && selectedMonth<=6)
     {
         alert("Please select month after june..!!");
     }
     else
     {
         createSlot(employeeId,selectedYear,selectedMonth);
     }	
...
// jugni have made change 20130621
function createSlot(empid,mon,yr){
 // alert("view timesheet"+empid);
    var r = $.ajax({
        type: 'POST',
        url: linkToCreateSlot,
        data: "employeeId="+empid+"&selectedMonth="+mon+"&selectedYear="+yr,
        async: false,
